...
| 插件实例内核（PP-M1） | 部分实现 | SQLite v4 实例模型和 `floatile instance create/list/get/configure/start/stop/delete` 已打通；创建/配置与 shell 恢复共用安装目录完整性和 Config JSON Schema 复验。shell 使用后台轮询+有界 UI 动作队列动态对齐 desired state，配置变更会仅重启对应实例，停止/删除会回收窗口与 runtime，缺失/篡改/配置非法的实例独立隔离且不会重试刷屏。同包两实例 CLI 二进制闭环、幂等 reconcile、迁移回滚和失败路径有自动测试；实际动态多窗口 Xvfb/真机证据、observed-state 界面与手动 retry 待做 |
