...
| 插件实例内核（PP-M1） | 部分实现 | SQLite v4 已提供实例创建/读取/枚举、Config/desired state 更新、generation 推进和删除；同 Installation 多实例、重启恢复、不复用 ID、迁移回滚与损坏行拒绝有测试。shell 在事件循环前按持久记录筛选 desired-running 实例、推进 generation、精确复核 plugin/version/digest，并以真实实例 ID/Config 启动独立窗口；篡改或缺失 Installation 及单窗口启动失败只隔离对应实例。尚缺实例管理入口和进程内动态启停 |
