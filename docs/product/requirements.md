...
| 包工具链 | 已实现，部分验证 | `floatile-cli` 已实现 `.floatile` 包校验核心（zip 预算、路径穿越/碰撞/symlink/zip-bomb 拒绝、manifest/UI IR/WASM world 校验与正反例 corpus）、原子安装与可恢复 journal；`floatile-core` 提供 detached DSSE/Ed25519 verifier与确定性升级/回滚权限规划；`floatile-store` 持久化 publisher/key trust、anti-rollback 水位与原子回滚审计。CLI 受信安装强制签名/撤销/anti-rollback，权限扩大需显式接受；stopped 实例可回滚到重新验签、migration 兼容且不恢复旧权限的历史 Installation并保持最高水位。`install.json` 明确记录 `unsigned|trusted`，Shell 对 trusted 安装在持久实例启动与 retry 前按当前 trust 重新验签，对 unsigned 保留显式本地开发降级路径；三宿主分发运行与公开发布仍受平台证据及许可门禁约束。 |
