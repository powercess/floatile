...
| Permission Broker / Operation（PP-M2） | 部分实现 | `floatile-services` Broker（deny-by-default 决策、scope/配额、脱敏审计 target `floatile::audit`）与 clock/log/timer/storage/metrics/theme 能力实现已完成并有测试；脱敏审计已落 SQLite `audit_log`（store v3 + shell 运行时经 `with_audit_listener`）；恶意插件 fixture（`plugins/evil-wasm`）与安全集成测试已实现（拒绝 + 审计 + 宿主存活）。ADR-0004 的宿主 Operation spike 已实现按实例/generation 的 identity、有界提交/完成/并发/结果预算、deadline、取消、typed one-shot result 和 runtime completion bridge，并覆盖拒绝、超时、取消、迟到结果、重启与过载；生产 WIT、SDK wrapper、guest completion dispatch 和动态撤权仍未实现，不能宣称插件已可使用异步 Operation；真实容量数据仍缺失 |
