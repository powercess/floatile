...
- 一个分支只承载一个需求、修复或治理目标。依赖升级单独成分支；不得夹带无关重构、格式化或
  其他协作者的修改。
- 所有 Git 修改操作（创建/切换分支、暂存、提交、rebase、删除分支、合并、交付）前都要检查
  `git status`、`git branch --show-current` 与相关 diff，不假设共享工作区的分支或文件状态；新建分支
  显式指定基线，不得依赖当前 HEAD。发现不属于当前任务的修改、预期外的分支/提交或并发写入时必须
  保留并避让，不得覆盖、回滚或带入提交。
